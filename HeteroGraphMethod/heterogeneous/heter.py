...
class HeterogeneousGraph(torch.nn.Module):
    def __init__(self, MODEL, c_node_in, c_node_out, c_edge, c_hidden = 8):
        ...
        self.model = MList([MList([MODEL((c_node_in[i],c_node_in[j]),c_hidden,c_edge[i][j]) for j in range(self.category)]) for i in range(self.category)])
    ...
    def forward(self,x,edge_index,edge_attr):
        ...
        o = [[self.model[i][j]((x[i],x[j]),edge_index[i][j],edge_attr[i][j],size=(None if i==j else (x[i].shape[0],x[j].shape[0]))) for j in range(self.category)] for i in range(self.category)]
